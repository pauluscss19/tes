...
import { updateCompanyJob } from "../../../services/jobs";
import {
  Info,
  MapPin,
  FileText,
...
/** Helper: ambil nilai ISO date (YYYY-MM-DD) dari berbagai format */
...
export default function EditJob() {
  ...
  // Data job dikirim dari JobPostings melalui navigate state
  const jobRow = location.state?.job;
  ...
  // Pre-fill form dari data job yang dikirim
  useEffect(() => {
    if (!jobRow) {
      // Kalau tidak ada data, kembali ke daftar lowongan
      navigate("/admin/mitra/lowongan", { replace: true });
      return;
    }

    const raw = jobRow.raw ?? jobRow;

    setFormData({
      judul_posisi:        raw?.judul_pekerjaan    || raw?.judul_posisi    || jobRow.title || "",
      tipe_pekerjaan:      raw?.tipe_pekerjaan     || raw?.kategori_pekerjaan || "full-time",
      lokasi:              raw?.lokasi             || "",
      tipe_magang:         raw?.pengaturan_kerja   || raw?.tipe_magang     || "onsite",
      gaji_min:            raw?.gaji_min           != null ? String(raw.gaji_min)  : "",
      gaji_max:            raw?.gaji_max           != null ? String(raw.gaji_max)  : "",
      deskripsi_pekerjaan: raw?.deskripsi_pekerjaan || "",
      persyaratan:         raw?.persyaratan        || "",
      deadline:            toDateInput(raw?.tgl_tutup_lamaran),
      mulai_kerja:         toDateInput(raw?.tgl_mulai_kerja),
  ...
  const buildPayload = () => ({
    judul_pekerjaan:      formData.judul_posisi.trim(),
    kategori_pekerjaan:   formData.tipe_pekerjaan,
    tipe_pekerjaan:       formData.tipe_pekerjaan,
    lokasi:               formData.lokasi.trim(),
    pengaturan_kerja:     formData.tipe_magang,
    gaji_min:             formData.gaji_min || null,
    gaji_max:             formData.gaji_max || null,
  ...
  const validateForm = () => {
    ...
    if (!payload.judul_pekerjaan || !payload.deskripsi_pekerjaan || !payload.persyaratan || !payload.lokasi) {
  ...
  const handleSubmitEdit = async () => {
    const jobId = jobRow?.backendId ?? jobRow?.raw?.id;
    if (!jobId) {
      setErrorMessage("ID lowongan tidak ditemukan. Silakan kembali dan coba lagi.");
    ...
    try {
      await updateCompanyJob(jobId, buildPayload());
      setShowSaveModal(false);
      navigate("/admin/mitra/lowongan");
    } catch (error) {
      setShowSaveModal(false);
      setErrorMessage(getApiErrorMessage(error, "Lowongan gagal diperbarui. Coba lagi sebentar."));
    } finally {
      ...
      <main className="create-job__main">
        ...
        <section className="create-job__section">
          <div className="create-job__breadcrumb">
            ...
            <span className="muted">›</span>
            <span className="active">EDIT LOWONGAN</span>
          </div>

          <div className="create-job__header">
            <h1 className="create-job__page-title">Edit Lowongan</h1>
            <p className="create-job__page-subtitle">
              Perbarui informasi lowongan yang sudah ada.
            </p>
          </div>
          ...
          <div className="create-job__card">
            ...
            <SectionTitle icon={<CalendarDays size={16} />} title="Tanggal Penting" />

            <div className="create-job__grid-two">
              <div>
                <Label>Tanggal Penutupan Lamaran</Label>
                <DateInput value={formData.deadline} onChange={updateField("deadline")} />
              </div>

              <div>
                <Label>Tanggal Mulai Kerja / Magang</Label>
                <DateInput value={formData.mulai_kerja} onChange={updateField("mulai_kerja")} />
              </div>
            </div>
